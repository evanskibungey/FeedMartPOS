    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-gradient-harvest rounded-2xl shadow-harvest p-8 text-white animate-fade-in-up">
                <div class="flex items-center justify-between">
                    <div class="space-y-2">
                        ...
                        <p class="text-harvest-50 text-lg">Here's what's happening with your business today</p>
                        <div class="flex items-center space-x-4 mt-4">
                            <div class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg">
                                <span class="text-sm font-semibold">Today's Revenue</span>
                                <p class="text-2xl font-bold">KES {{ number_format($revenueStats['today_revenue'], 2) }}</p>
                            </div>
                            <div class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg">
                                <span class="text-sm font-semibold">Today's Transactions</span>
                                <p class="text-2xl font-bold">{{ $revenueStats['today_transactions'] }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-slide-in-right">
                ...
                <!-- Total Revenue -->
                <div class="stat-card stat-card-harvest group cursor-pointer hover:scale-105 transition-transform duration-300">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm font-medium mb-1">Total Revenue</p>
                            <p class="text-3xl font-bold text-gray-800">KES {{ number_format($revenueStats['total_revenue'], 0) }}</p>
                            <p class="text-sm font-medium mt-1 flex items-center {{ $monthComparison['growth_percentage'] >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                ...
                                {{ number_format(abs($monthComparison['growth_percentage']), 1) }}% vs last month
                            </p>
                        </div>
                ...
                <!-- Total Transactions -->
                <div class="stat-card stat-card-agri group cursor-pointer hover:scale-105 transition-transform duration-300">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm font-medium mb-1">Total Transactions</p>
                            <p class="text-3xl font-bold text-gray-800">{{ number_format($revenueStats['total_transactions']) }}</p>
                            <p class="text-agri-600 text-sm font-medium mt-1">
                ...
                <!-- Total Products -->
                <div class="stat-card stat-card-sky group cursor-pointer hover:scale-105 transition-transform duration-300">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm font-medium mb-1">Total Products</p>
                            <p class="text-3xl font-bold text-gray-800">{{ $productStats['total_products'] }}</p>
                            <p class="text-sky-600 text-sm font-medium mt-1">
                ...
                <!-- Pending Orders -->
                <div class="stat-card group cursor-pointer hover:scale-105 transition-transform duration-300 {{ $orderStats['pending_orders'] > 0 ? 'border-l-4 border-yellow-500' : '' }}">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-gray-600 text-sm font-medium mb-1">Pending Orders</p>
                            <p class="text-3xl font-bold text-gray-800">{{ $orderStats['pending_orders'] }}</p>
                            <p class="text-yellow-600 text-sm font-medium mt-1">
                                Needs attention
                            </p>
                        </div>
                        <div class="h-16 w-16 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
